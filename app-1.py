...
function loadMacro() {
  fetch('/macro')
    ...
    .then(function(d){
      ...
      if(d.dxy){
        var el = document.getElementById('dxy');
        el.textContent = d.dxy.price + ' ' + d.dxy.arrow;
        el.className = d.dxy.bearish_gold ? 'up' : 'down';
      }
      if(d.us10y){
        var el2 = document.getElementById('us10y');
        el2.textContent = d.us10y.yield + '% ' + d.us10y.arrow;
        el2.className = d.us10y.bearish_gold ? 'up' : 'down';
      }
      if(d.macro_bias){
        var el3 = document.getElementById('bias');
        el3.textContent = d.macro_bias;
        el3.className = d.macro_bias === 'BEARISH' ? 'up' : d.macro_bias === 'BULLISH' ? 'down' : 'neutral';
      }
      if(d.killzone){
        var kz = document.getElementById('killzone-bar');
    ...
    .catch(function(e){ console.log('Macro error:', e); });
}
loadMacro();
...
function setDir(d) {
  dir = d;
  document.getElementById('s').className = 'btn sell-btn' + (d === 'SELL' ? ' on' : '');
  document.getElementById('b').className = 'btn buy-btn' + (d === 'BUY' ? ' on' : '');
}

// Set timeframe
function setTF(t) {
  tf = t;
  document.getElementById('t1').className = 'tf-btn' + (t === '1m' ? ' on' : '');
  document.getElementById('t5').className = 'tf-btn' + (t === '5m' ? ' on' : '');
  document.getElementById('t15').className = 'tf-btn' + (t === '15m' ? ' on' : '');
  document.getElementById('t1h').className = 'tf-btn' + (t === '1h' ? ' on' : '');
}
...
function tog(label) {
  ...
  label.className = 'sig' + (cb.checked ? ' on' : '');
}
...
function analyze() {
  if(!dir) { showStatus('❌ Please select SELL or BUY first!', 'err'); return; }
  if(!tf)  { showStatus('❌ Please select a timeframe!', 'err'); return; }

  var sigs = getSigs();
  if(sigs.length === 0) { showStatus('❌ Please select at least one signal!', 'err'); return; }

  var sh = document.getElementById('sh').value;
  var sl = document.getElementById('sl').value;

  showStatus('🔍 Fetching live data and analyzing...', 'loading');
  ...
  .then(function(d){
    ...
    if(d.success){
      ...
      showStatus('❌ Error: ' + (d.error || 'Unknown error'), 'err');
    }
  })
  .catch(function(e){
    document.getElementById('go').disabled = false;
